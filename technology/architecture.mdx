---
title: "SURCHI Architecture: Multi-Chain Analytics at Scale"
...
<CardGroup cols={2}>
  ...
  <Card title="Blockchain Indexers" icon="database">
    Per-chain indexing services that track every block and transaction in real time across all supported networks. They parse on-chain events and normalise the data so it is immediately available for analytics queries and real-time alerts.
  </Card>
  ...
  <Card title="Unified Data Layer" icon="layer-group">
    Abstracts away the differences between chain-specific data formats so that every API response follows a consistent, predictable schema — regardless of whether the underlying data originated on Ethereum, Solana, BNB Chain, or any other supported network.
  </Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="Indexer Detects & Parses">
    The blockchain indexer decodes every transaction and log in the block, extracts token transfers, DEX swaps, contract interactions, and wallet balance changes, and prepares them for normalisation and analysis.
  </Step>
  ...
  <Step title="Pushed to Users">
    Processed data becomes immediately queryable, cache is refreshed, and any subscribers with matching WebSocket filters receive a real-time push notification. REST API queries against this data return within milliseconds.
  </Step>
</Steps>
