---
title: "How SURCHI Works: Real-Time Multi-Chain Analytics"
...
<Steps>
  ...
  <Step title="Index & Normalize">
    Raw blockchain data varies significantly in structure between chains (EVM vs. Solana vs. others). SURCHI's normalization layer translates all incoming data into a unified schema — standardizing address formats, token decimals, timestamp precision, and event types. This normalized data is written to SURCHI's indexed data store, where it becomes immediately queryable by the API and frontend.
  </Step>
